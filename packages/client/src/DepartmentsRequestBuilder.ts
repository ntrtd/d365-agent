...
> extends RequestBuilder<Departments<T>, T> {
   ...
   * @param partyNumber Key property. See {@link Departments.partyNumber}.
   ...
   */
  delete(partyNumber: string): DeleteRequestBuilder<Departments<T>, T>;
  /**
   * Returns a request builder for deleting an entity of type `Departments`.
  ...
  delete(partyNumberOrEntity: any): DeleteRequestBuilder<Departments<T>, T> {
    return new DeleteRequestBuilder<Departments<T>, T>(
      this.entityApi,
      partyNumberOrEntity instanceof Departments
        ? partyNumberOrEntity
